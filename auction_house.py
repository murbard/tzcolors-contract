...
class TZColorsFA2(sp.Contract):
    def __init__(self, initial_auction_house_address):
        self.init(
            ...
            royalty_params = sp.big_map(tkey=sp.TNat, tvalue=Royalty.get_type()),
...
def test():
    ...
    scenario += fa2.transfer([BatchTransfer.item(alice.address, [sp.record(to_=bob.address, token_id=0, amount=1)])]).run(sender=auction_house.address, valid=False)

    scenario.p("Auction house can transfer Alice's token after allowance")
    scenario += fa2.update_operators([sp.variant('add_operator', sp.record(owner=alice.address,operator=auction_house.address,token_id=0))]).run(sender=alice)
    scenario += fa2.transfer([BatchTransfer.item(alice.address, [sp.record(to_=bob.address, token_id=0, amount=1)])]).run(sender=auction_house.address)
    ...
    scenario.p("Bob cannot transfer as owner")
    auction_id = sp.nat(0) # we can reuse 0
    scenario += fa2.transfer([BatchTransfer.item(bob.address, [sp.record(to_=dan.address, token_id=0, amount=1)])]).run(sender=bob, valid=False)
